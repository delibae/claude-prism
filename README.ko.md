...
ClaudePrism은 다른 접근 방식을 취합니다: **파일이 내 컴퓨터에 남습니다.**
...
문서는 로컬 디스크에 저장됩니다. ClaudePrism은 **Claude Code**를 로컬 서브프로세스로 실행하며 — 파일이 원격으로 업로드되거나 저장되지 않습니다.

> **참고:** 두 도구 모두 AI 추론을 위해 프롬프트와 파일 내용을 클라우드 API로 전송합니다. 차이점은 파일이 저장되는 위치입니다 — OpenAI Prism은 파일을 자사 서버에 보관하고, ClaudePrism은 사용자의 디스크에 보관합니다. 자세한 내용은 [Claude Code 데이터 사용 정책](https://code.claude.com/docs/en/data-usage)을 참고하세요.
